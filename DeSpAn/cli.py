...
def main() -> int:
    ...
    pcd_e2_path_boxcut = RUN_CFG.paths.intermediate_results / f"02b_{RUN_CFG.project_meta.epoch2_name}_boxcut.ply"
    ...
    offset_xy = -np.round(np.array(border_common.exterior.centroid.coords.xy).T.squeeze())

    pcd_e1_path_bordercut = RUN_CFG.paths.intermediate_results / f"03a_{RUN_CFG.project_meta.epoch1_name}_bordercut.ply"
    pcd_e2_path_bordercut = RUN_CFG.paths.intermediate_results / f"03b_{RUN_CFG.project_meta.epoch2_name}_bordercut.ply"

    print("Running border cut on first point cloud")
    cc_call_e1 = subprocess.run([RUN_CFG.paths.CC_exe, '-SILENT',
                                 '-LOG_FILE', f"{RUN_CFG.paths.intermediate_results / 'boxcut_e1.log'}",
                                 '-C_EXPORT_FMT', 'PLY',
                                 '-AUTO_SAVE', 'OFF',
                                 '-O', '-GLOBAL_SHIFT', *[f"{x:0.3f}" for x in offset_xy], "0.0",
                                 f"{pcd_e1_path_boxcut}",
                                 '-CROP2D', 'Z', str(border_xy.shape[0]),
                                 *[f"{x:0.3f}" for x in (border_xy + offset_xy).flatten()],
                                 '-SAVE_CLOUDS', 'FILE', f"{pcd_e1_path_bordercut}",
                                 ],
                                shell=False, check=True, text=True, capture_output=True)

    print("Running border cut on second point cloud")
    cc_call_e2 = subprocess.run([RUN_CFG.paths.CC_exe, '-SILENT',
                                 '-LOG_FILE', f"{RUN_CFG.paths.intermediate_results / 'boxcut_e2.log'}",
                                 '-C_EXPORT_FMT', 'PLY',
                                 '-AUTO_SAVE', 'OFF',
                                 '-O', '-GLOBAL_SHIFT', *[f"{x:0.3f}" for x in offset_xy], "0.0",
                                 ...
                                 '-CROP2D', 'Z', str(border_xy.shape[0]),
                                 *[f"{x:0.3f}" for x in (border_xy + offset_xy).flatten()],
                                 '-SAVE_CLOUDS', 'FILE', f"{pcd_e2_path_bordercut}",
                                 ],
                                shell=False, check=True, text=True, capture_output=True)

    print("Running M3C2")

    cc_m3c2 = subprocess.run([RUN_CFG.paths.CC_exe, '-SILENT', '-NO_TIMESTAMP',
                              '-LOG_FILE', f"{RUN_CFG.paths.intermediate_results / 'log_m3c2.log'}",
                              '-C_EXPORT_FMT', 'PLY',
                              '-AUTO_SAVE', 'OFF',
                              '-O', '-GLOBAL_SHIFT', *[f"{x:.3f}" for x in offset_xy], "0.0", f"{pcd_e1_path_bordercut}",
                              '-O', '-GLOBAL_SHIFT', *[f"{x:.3f}" for x in offset_xy], "0.0", f"{pcd_e2_path_bordercut}",
                              '-M3C2', f"{RUN_CFG.paths.m3c2_settings}",
                              '-SET_ACTIVE_SF', '8',
                              '-SF_COLOR_SCALE', f"{RUN_CFG.paths.hsv_settings}",
